...
public class PriceWidgetProvider extends AppWidgetProvider {
    ...
    public static void updateAllWidgets(Context ctx) {
        try {
            ...
            // Large (3-row) widgets
            ComponentName cnLarge = new ComponentName(ctx, PriceWidgetProvider.class);
            int[] largeIds = mgr.getAppWidgetIds(cnLarge);
            for (int id : largeIds) updateWidget(ctx, mgr, id);
            // Small (single-price) widgets
            ComponentName cnSmall = new ComponentName(ctx, PriceWidgetProviderSmall.class);
            int[] smallIds = mgr.getAppWidgetIds(cnSmall);
            for (int id : smallIds) PriceWidgetProviderSmall.updateSmallWidget(ctx, mgr, id);
        } catch (Exception e) {
            Log.e(TAG, "updateAllWidgets failed", e);
        }
    }

    protected static void scheduleAlarm(Context ctx) {
        try {
