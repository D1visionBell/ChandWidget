...
public class Prefs {
    ...
    private static final String SMALL_MULTI = "small_multi_";   // per-widget bool
    ...
    public static void removeWidget(Context ctx, int appWidgetId) {
        ...
        ed.remove(SMALL_MULTI + appWidgetId);
        ed.apply();
    }

    // ── Small widget display mode (single price vs 3-row) ─────────────────────
    public static boolean getSmallWidgetMultiRow(Context ctx, int appWidgetId) {
        return get(ctx).getBoolean(SMALL_MULTI + appWidgetId, false);
    }

    public static void setSmallWidgetMultiRow(Context ctx, int appWidgetId, boolean multi) {
        get(ctx).edit().putBoolean(SMALL_MULTI + appWidgetId, multi).apply();
    }
